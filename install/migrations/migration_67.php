...
try {
    ...
    createTempTables();
    processSimulations($timestamp);
    processRecent($timestamp);
...
function processSimulations($timestamp)
{
    $db = Database::get();
...
function processRecent($timestamp)
{
    $db = Database::get();
    $sql = "SELECT * FROM %%RW%% WHERE `time` > :sevenDays;";
...
function processTop($universe)
{
    ...
    insertTopkbValues($memorials);
}

function insertTopkbValues(array $topkbs)
{
    $db = Database::get();
    foreach ($topkbs as $topkb) {
        ...
        $topkbUsers = $db->select($sql, [
            ':rid' => $topkb['rid'],
        ]);
    
        foreach ($topkbUsers as $user) {
