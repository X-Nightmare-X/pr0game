...
if (!allowedTo(str_replace([dirname(__FILE__), '\\', '/', '.php'], '', __FILE__)) || $_GET['sid'] != session_id()) {
    exit;
}
...
function ShowRightsPage()
{
    ...
    switch ($mode) {
        case 'rights':
            ...
            if ($_POST) {
                ...
                if ($_POST['action'] == 'send') {
                    $GLOBALS['DATABASE']->query("UPDATE " . USERS . " SET `rights` = '"
                        . serialize(array_map('intval', $_POST['rights'])) . "' WHERE `id` = '" . $id . "';");
                }
            ...
            $QueryUsers =   $GLOBALS['DATABASE']->query("SELECT `id`, `username`, `authlevel` FROM " . USERS
                . " WHERE `universe` = '" . Universe::getEmulated() . "'" . $WHEREUSERS . ";");

            $UserList   = "";
            while ($List = $GLOBALS['DATABASE']->fetch_array($QueryUsers)) {
                $UserList .= '<option value="' . $List['id'] . '">' . $List['username'] . '&nbsp;&nbsp;(' . $LNG['rank_'
                    . $List['authlevel']] . ')</option>';
            }
        ...
        case 'users':
            ...
            if ($_POST) {
                ...
                $GLOBALS['DATABASE']->multi_query("UPDATE " . USERS . " SET `authlevel` = '"
                    . HTTP::_GP('authlevel', 0) . "' WHERE `id` = '" . $id . "';");
            ...
            $UserList   = "";
            while ($List = $GLOBALS['DATABASE']->fetch_array($QueryUsers)) {
                $UserList   .=  '<option value="' . $List['id'] . '">' . $List['username'] . '&nbsp;&nbsp;('
                    . $LNG['rank_' . $List['authlevel']] . ')</option>';
            }
