...
function ShowCronjobEdit($post_id)
{
    ...
    if (count($error_msg) == 0) {
        if ($post_id != 0) {
            $GLOBALS['DATABASE']->query("UPDATE " . CRONJOBS . " SET name = '"
                . $GLOBALS['DATABASE']->sql_escape($post_name) . "', min = '" . $post_min . "', hours = '" . $post_hours
                . "', month = '" . $post_month . "', dow = '" . $post_dow . "', dom = '" . $post_dom . "', class = '"
                . $GLOBALS['DATABASE']->sql_escape($post_class) . "' WHERE cronjobID = $post_id;");
        } else {
            $GLOBALS['DATABASE']->query("INSERT INTO " . CRONJOBS . " SET name = '"
                . $GLOBALS['DATABASE']->sql_escape($post_name) . "', min = '" . $post_min . "', hours = '" . $post_hours
                . "', month = '" . $post_month . "', dow = '" . $post_dow . "', dom = '" . $post_dom . "', class = '"
                . $GLOBALS['DATABASE']->sql_escape($post_class) . "';");
        }
...
{
    $GLOBALS['DATABASE']->query("UPDATE " . CRONJOBS . " SET `lock` = MD5(UNIX_TIMESTAMP()) WHERE cronjobID = "
        . $cronjobId . ";");
    HTTP::redirectTo('admin.php?page=cronjob');
}
...
{
    $GLOBALS['DATABASE']->query("UPDATE " . CRONJOBS . " SET `isActive` = " . HTTP::_GP('enable', 0)
...
function ShowCronjobOverview()
{
    ...
    while ($CronjobRow = $GLOBALS['DATABASE']->fetch_array($data)) {
        $cronjobLogLastExecSql = "SELECT MAX(`executionTime`) AS lastExecution FROM " . CRONJOBS_LOG
            . " WHERE cronjobID = " . $CronjobRow['cronjobID'] . ";";
...
function ShowCronjobDetail($detail, $error_msg = null)
{
    ...
    if ($detail != 0) {
        $CronjobRow     = $GLOBALS['DATABASE']->uniquequery("SELECT * FROM " . CRONJOBS . " WHERE cronjobID = "
            . $detail . "");
        $template->assign_vars([
            'id'            => $CronjobRow['cronjobID'],
            'name'          => isset($_POST['name']) ? HTTP::_GP('name', '') : $CronjobRow['name'],
            'min'           => isset($_POST['min_all']) ? [0 => '*'] : (
                isset($_POST['min']) ? HTTP::_GP('min', []) : getCronjobTimes($CronjobRow['min'], 59)
            ),
            'hours'         => isset($_POST['hours_all']) ? [0 => '*'] : (
                isset($_POST['hours']) ? HTTP::_GP('hours', []) : getCronjobTimes($CronjobRow['hours'], 23)
            ),
            'dom'           => isset($_POST['dom_all']) ? [0 => '*'] : (
                isset($_POST['dom']) ? HTTP::_GP('dom', []) : getCronjobTimes($CronjobRow['dom'], 31)
            ),
            'month'         => isset($_POST['month_all']) ? [0 => '*'] : (
                isset($_POST['month']) ? HTTP::_GP('month', []) : getCronjobTimes($CronjobRow['month'], 12)
            ),
            'dow'           => isset($_POST['dow_all']) ? [0 => '*'] : (
                isset($_POST['dow']) ? HTTP::_GP('dow', []) : getCronjobTimes($CronjobRow['dow'], 6)
            ),
