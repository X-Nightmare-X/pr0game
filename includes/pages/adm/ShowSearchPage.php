...
function ShowSearchPage()
{
    ...
    if (!empty($SearchFile)) {
        ...
        if (in_array($SearchFile, $ArrayUsers)) {
            ...
            $SpecifyItems   = "p.id,p.name,CONCAT(u.username, ' (ID:&nbsp;', p.id_owner, ')'),p.last_update,p.galaxy,"
                . "p.system,p.planet,p.id_luna";
...
function MyCrazyLittleSearch(
    ...
    if ($CountQuery['total'] > 0) {
        ...
        if ($NumberOfPages > 1) {
            ...
            $Search['PAGES']    = '<tr><td colspan="3" style="color:#00CC33;border: 1px lime solid;text-align:center;">'
                . $BEFORE . '&nbsp;' . $PAGEE . '&nbsp;' . $NEXT . '</td></tr>';
        }
        ...
        while ($WhileResult = $GLOBALS['DATABASE']->fetch_num($FinalQuery)) {
            ...
            if ($Table == "users") {
                if (allowedTo('ShowQuickEditorPage')) {
                    $Search['LIST'] .= "<td><a href=\"javascript:openEdit('" . $WhileResult[0]
                        . "', 'player');\" border=\"0\"><img title=\"" . $WhileResult[1]
                        . "\" src=\"./styles/resource/images/admin/GO.png\"></a></d>";
                }

                if ($USER['authlevel'] == AUTH_ADM) {
                    if ($WhileResult[0] != $USER['id'] || $WhileResult[0] != ROOT_USER) {
                        $DELETEBUTTON = '<a href="?page=search&amp;delete=user&amp;user=' . $WhileResult[0]
                            . '" border="0" onclick="return confirm(\'' . $LNG['ul_sure_you_want_dlte'] . ' '
                            . $WhileResult[1] . '?\');"><img src="./styles/resource/images/alliance/CLOSE.png"'
                            . ' width="16" height="16" title=' . $WhileResult[1] . '></a>';
                    } else {
                        $DELETEBUTTON =  '-';
                    }
            ...
            if ($Table == "planets p") {
                if (allowedTo('ShowQuickEditorPage')) {
                    $Search['LIST'] .= "<td><a href=\"javascript:openEdit('" . $WhileResult[0]
                        . "', 'planet');\" border=\"0\"><img src=\"./styles/resource/images/admin/GO.png\" title="
                        . $LNG['se_search_edit'] . "></a></td>";
                }

                if ($USER['authlevel'] == AUTH_ADM) {
                    $Search['LIST'] .= '<td><a href="?page=search&amp;delete=planet&amp;planet=' . $WhileResult[0]
                        . '" border="0" onclick="return confirm(\'' . $LNG['se_confirm_planet'] . ' ' . $WhileResult[1]
                        . '\');"><img src="./styles/resource/images/alliance/CLOSE.png" width="16" height="16" title='
                        . $LNG['button_delete'] . '></a></td>';
                }
            }
        ...
        $Result['LIST'] .= "<tr><td style='color:#00CC33;border: 2px red solid;' height='25px'><font color=red>"
            . $LNG['se_no_data'] . "</font></td></tr>";
        $Result['LIST'] .= "</table>";
        return $Result;
    }
}
