...
{block name="content"}
  ...
  <table class="table569">
    <tr>
      <th colspan="{8+{$colespanAdd}}">{$LNG.gl_solar_system} {$galaxy}:{$system}</th>
    </tr>
    <tr>
        ...
        {else}
              ...
              {if $currentPlanet.wreckfield}
                <a class="tooltip_sticky" data-tooltip-content="
                  <table style='width:240px'>
                    <tr>
                      <th colspan='2'>{$LNG.gl_wreckfield} [{$galaxy}:{$system}:{$planet}]</th>
                    </tr>
                    <tr>
                      <td style='width:80px'><img src='{$dpath}planeten/wreckfield.jpg' height='75' style='width:75'></td>
                      <td>
                        <table style='width:100%'>
                          <tr><th colspan='2'>{$LNG.lv_ships}:</th></tr>
                          {foreach $currentPlanet.wreckfield as $shipID => $amount}
                            <tr><td>{$LNG.tech.{$shipID}}: </td><td>{number_format({$amount}, 0, ",", ".")}</td></tr>
                          {/foreach}
                        </table>
                      </td>
                    </tr>
                  </table>">
                  <img src="{$dpath}planeten/wreckfield.jpg" height="22" width="22" alt="">
                </a>
              {/if}
            </td>
          {/if}
          ...
          <td>
            <a class="tooltip_sticky" data-tooltip-content="<table style='width:240px'><tr><th colspan='2'>{$currentPlanet.user.playerrank}</th></tr><tr>{if !$currentPlanet.user.isBuddy && !$currentPlanet.ownPlanet}<tr><td><a href='#' onclick='return Dialog.Buddy({$currentPlanet.user.id})'>{$LNG.gl_buddy_request}</a></td></tr>{/if}<tr><td><a href='#' playerid='{$currentPlanet.user.id}' onclick='return Dialog.Playercard({$currentPlanet.user.id});'>{$LNG.gl_playercard}</a></td></tr><tr><td><a href='?page=statistics&amp;who=1&amp;start={$currentPlanet.user.rank}'>{$LNG.gl_see_on_stats}</a></td></tr></table>">
              ...
              {/foreach} galaxy-username
              {if $currentPlanet.user.isBuddy} galaxy-friend
              {/if}">{$currentPlanet.user.username}</span>
              ...
              {if !empty($currentPlanet.user.class)}
                <span>(</span>{foreach $currentPlanet.user.class as $class}
                  {if !$class@first}&nbsp;
                  {/if}<span class="galaxy-short-{$class} galaxy-short">{$ShortStatus.$class}</span>
                {/foreach}<span>)</span>
              {/if}
            </a>
          </td>
    ...
    <tr>
      <td>{$max_planets + 1}</td>
      <td colspan="{7+{$colespanAdd}}"><a href="?page=fleetTable&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$max_planets + 1}&amp;planettype=1&amp;target_mission=15">{$LNG.gl_out_space}</a></td>
    </tr>
    <tr>
      <td>Trade</td>
      <td colspan="{7+{$colespanAdd}}"><a href="?page=fleetTable&amp;galaxy={$galaxy}&amp;system={$system}&amp;planet={$max_planets + 2}&amp;planettype=1&amp;target_mission=16">{$LNG.gl_trade_space}</a></td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="4"><span id="missiles">{number_format($currentmip, 0, ",", ".")}</span> {$LNG.gl_avaible_missiles}</td>
      <td colspan="{4+{$colespanAdd}}"><span id="slots">{$maxfleetcount}</span>/{$fleetmax} {$LNG.gl_fleets}</td>
    </tr>
    <tr data-info="spy_recs">
      <td colspan="4">
        <span id="elementID210">{number_format($spyprobes, 0, ",", ".")}</span> {$LNG.gl_avaible_spyprobes}
      </td>
      <td colspan="{4+{$colespanAdd}}">
    ...
    <tr data-info="galatool">
      ...
      <td colspan="1"><button id="gala_exportcsv">Export CSV</button></td>
      <td colspan="{3+{$colespanAdd}}">Import JSON: <input id="gala_upload" type="file" accept=".json"></td>
