...
<table>
	<tbody>
		...
		<tr>
			...
			<td>{array_sum($resourceArray)|number_format} {if in_array($elementID, array(901,902,903))}<span style="color:lime">{array_sum($planetList.resourcePerHour[$elementID])|number_format}/h</span>{/if}</td>
			{foreach $resourceArray as $planetID => $resource}
				<td>{$resource|number_format} {if in_array($elementID, array(901,902,903)) && $planetList.planet_type[$planetID] == 1}<span style="color:lime">{$planetList.resourcePerHour[$elementID][$planetID]|number_format}/h</span>{/if}</td>
			{/foreach}
		</tr>
		{/foreach}
		<tr>
		...
		<tr>
			<td><a href='#' onclick='return Dialog.info({$elementID})' class='tooltip' data-tooltip-content="<table><tr><th>{$LNG.tech.{$elementID}}</th></tr><tr><table class='hoverinfo'><tr><td><img src='{$dpath}gebaeude/{$elementID}.{if $elementID >=600 && $elementID <= 699}jpg{else}gif{/if}'></td><td>{$LNG.shortDescription.$elementID}</td></tr></table></tr></table>">{$LNG.tech.$elementID}</a></td>
			<td>{array_sum($buildArray)|number_format}</td>
		...
		<tr>
			<td><a href='#' onclick='return Dialog.info({$elementID})' class='tooltip' data-tooltip-content="<table><tr><th>{$LNG.tech.{$elementID}}</th></tr><tr><table class='hoverinfo'><tr><td><img src='{$dpath}gebaeude/{$elementID}.{if $elementID >=600 && $elementID <= 699}jpg{else}gif{/if}'></td><td>{$LNG.shortDescription.$elementID}</td></tr></table></tr></table>">{$LNG.tech.$elementID}</a></td>
			<td>{array_sum($fleetArray)|number_format}</td>
