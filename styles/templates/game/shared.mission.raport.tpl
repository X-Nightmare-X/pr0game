					...
					<td>
						...
						{if !empty($Player.ships)}
							...
							<tr>
								...
								{foreach $Player.ships as $ShipID => $ShipData}
									<td class="transparent">{$ShipData[0]|number_format}
										{if $Round > 0 && $ShipData[0] - $Raport.rounds[0].attacker[$PlayerNr].ships[$ShipID][0]}
											<br><span style="color:#ee4d2e"> ({($ShipData[0] - $Raport.rounds[0].attacker[$PlayerNr].ships[$ShipID][0])|number_format})</span>
										{/if}
									</td>
								{/foreach}
							</tr>
							{if !isset($Info)}
								<tr>
									...
									{foreach $Player.ships as $ShipID => $ShipData}
									<td class="transparent">{$ShipData[1]|number_format}</td>
									{/foreach}
								</tr>
								<tr>
									<td class="transparent">{$LNG.sys_ship_shield}</td>
									{foreach $Player.ships as $ShipID => $ShipData}
									<td class="transparent">{$ShipData[2]|number_format}</td>
									{/foreach}
								</tr>
								<tr>
									<td class="transparent">{$LNG.sys_ship_armour}</td>
									{foreach $Player.ships as $ShipID => $ShipData}
									<td class="transparent">{$ShipData[3]|number_format}</td>
									{/foreach}
								</tr>
							{/if}
						{else}
							...
							<tr>
								...
								<td class="transparent">{$ShipData[0]|number_format}
									{if $Round > 0 && $ShipData[0] - $Raport.rounds[0].defender[$PlayerNr].ships[$ShipID][0]}
										<br><span style="color:#ee4d2e"> ({($ShipData[0] - $Raport.rounds[0].defender[$PlayerNr].ships[$ShipID][0])|number_format})</span>
									{/if}
								</td>
								{/foreach}
							</tr>
...
{if !$RoundInfo@last}
{$LNG.fleet_attack_1} {$RoundInfo.info[0]|number_format} {$LNG.fleet_attack_2} {$RoundInfo.info[3]|number_format} {$LNG.damage}<br>
{$LNG.fleet_defs_1} {$RoundInfo.info[2]|number_format} {$LNG.fleet_defs_2} {$RoundInfo.info[1]|number_format} {$LNG.damage}<br><hr>
{/if}
{/foreach}
<br><br>
{if $Raport.result == "a"}
{$LNG.sys_attacker_won}<br>
{$LNG.sys_stealed_ressources} {foreach $Raport.steal as $elementID => $amount}{$amount|number_format} {$LNG.tech.$elementID}{if ($amount@index + 2) == count($Raport.steal)} {$LNG.sys_and} {elseif !$amount@last}, {/if}{/foreach}
...
{/if}
<br><br>
{$LNG.sys_attacker_lostunits} {$Raport['units'][0]|number_format} {$LNG.sys_units}<br>
{$LNG.sys_defender_lostunits} {$Raport['units'][1]|number_format} {$LNG.sys_units}<br>
{$LNG.debree_field_1} {foreach $Raport.debris as $elementID => $amount}{$amount|number_format} {$LNG.tech.$elementID}{if ($amount@index + 2) == count($Raport.debris)} {$LNG.sys_and} {elseif !$amount@last}, {/if}{/foreach}{$LNG.debree_field_2}<br><br>
{if $Raport.mode == 1}
	{* Destruction *}
