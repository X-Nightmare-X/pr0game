    ...
    {foreach $elementIDs as $elementID => $amount}
      ...
      {/if}
        ...
        <div class="spyRaportContainerCell" data-info="{$Class}_{$elementID}">
          {number_format($amount, 0, ",", ".")}{if isset($repair_order[$elementID])} <span class="colorPositive">(+ {$repair_order[$elementID]})</span>{/if}
        </div>
      {if ($amount@iteration % 2) === 0}</div>{/if}
    {/foreach}
	</div>
  {if !empty($repair_order) && $Class == 200}
    <div><span class="colorPositive">(+ X)</span> {$LNG.sys_mess_spy_repair_order}</div>
  {/if}
  <br>
	{/foreach}
	<div class="spyRaportFooter">
		...
		<br>{if $totalShipDefCount > 0 && $targetChance >= $spyChance}<span class="colorNegative">{$LNG.sys_mess_spy_destroyed}</span>{else}{sprintf($LNG.sys_mess_spy_lostproba, $targetChance)}{/if}
